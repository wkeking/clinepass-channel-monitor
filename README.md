...
- **官方口径**（近 1 小时 / 近 24 小时）：请求数、总 Token 数、缓存命中率都取 Cline 官方的逐条计费记录，覆盖整个账号（含 Cline IDE 等其他客户端），因此会大于本机 CPA 的记录。近 7 天窗口的**总 Token 数与成本**也来自官方（按自然日汇总），但**请求数与缓存命中率**官方没有对应数据，这两个仍是本机口径。
- **本机口径**（平均延时 / 生成速度，以及官方不可用时的全部数值）：只统计经过本 CPA 的 Cline 请求。官方接口没有延时与生成速度字段，这两块永远是本机口径。
